...
def create_feature_selection_data(df_policy, df_claim, reduction=10):
    '''
    In:
        DataFrame(df_policy),
        DataFrame(df_claim),
    ...
    y_valid = read_data('y_valid_bs.csv', 'interim')
    y_test = read_data('y_test_bs.csv', 'interim')
    ...
    write_test_data(X_train, "X_train_new.csv")
    write_test_data(y_train, "y_train_new.csv")
    write_test_data(X_valid, "X_valid_new.csv")
    write_test_data(y_valid, "y_valid_new.csv")
    write_test_data(X_train_all, "X_train_all_new.csv")
    write_test_data(y_train_all, "y_train_all_new.csv")
    write_test_data(X_test, "X_test_new.csv")
    write_test_data(y_test, "y_test_new.csv")
...
def get_bs_quick_mae(params):
    ...
    '''
    X_train = read_data('X_train_new.csv', 'interim')
    y_train = read_data('y_train_new.csv', 'interim')
    X_valid = read_data('X_valid_new.csv', 'interim')
    y_valid = read_data('y_valid_new.csv', 'interim')
    ...
    write_test_data(valid_pred, 'fit_valid_new.csv')
    ...
    write_test_data(train_pred, 'fit_train_new.csv')
...
def get_bs_quick_submission(params, prelim):
    ...
    Description:
        calculate quick mae on validation set
    '''
    X_train = read_data('X_train_all_new.csv', 'interim')
    y_train = read_data('y_train_all_new.csv', 'interim')
    X_valid = read_data('X_test_new.csv', 'interim')
    y_valid = read_data('y_test_new.csv', 'interim')
...
######## read/write func ########
def read_data(file_name, path, index_col='Policy_Number'):
    '''
    In: file_name
    ...
    raw_data_path = os.path.join(os.path.dirname('__file__'), os.path.pardir, os.path.pardir, 'data', path)
...
def write_test_data(df, file_name, path='interim'):
    '''
    In:
        DataFrame(df),
        str(file_name),
    ...
    '''
    interim_data_path = os.path.join(os.path.dirname('__file__'), os.path.pardir, os.path.pardir, 'data', path)
...
def demo(reduction=1):
    ...
    df_claim = read_data('claim_0702.csv', 'raw')
    df_policy = read_data('policy_0702.csv', 'raw')

    create_feature_selection_data(
        df_policy, df_claim, reduction=reduction
    )
